...
#blue fruit (different weight)
blue_fruit = snake.copy()
blue_fruit.center = randompos()
def blue_spawn():
...
needed = 12
#blue fruit stuff
blue_fruit_timer = None
blue_fruit_duration = 7000
...
while True:
    ...
    if snake.center == fruit.center:
        ...
        while fruit.collidelist(segments) != -1:
            fruit.center = randompos()
        # Decide to activate blue fruit
        if blue_spawn():
            blue_fruit_active = True
            blue_fruit.center = randompos()
            while blue_fruit.collidelist(segments + [fruit]) != -1:
                blue_fruit.center = randompos()
    if blue_fruit_active and snake.center == blue_fruit.center:
        blue_fruit_active = False  # Deactivate after eating
        length += 3
        score += 3
        blue_fruit_timer = None
    if blue_fruit_active:
        pygame.draw.rect(screen, 'blue', blue_fruit)
    #blue fruit timer
    if blue_spawn() and not blue_fruit_active:
        blue_fruit_active = True
        blue_fruit.center = randompos()
        while blue_fruit.collidelist(segments + [fruit]) != -1:
            blue_fruit.center = randompos()
        blue_fruit_timer = pygame.time.get_ticks() 
    if blue_fruit_active:
        current_time = pygame.time.get_ticks()
        if current_time - blue_fruit_timer > blue_fruit_duration:
            blue_fruit_active = False  # Deactivate the blue fruit because time is up
        else:
            pygame.draw.rect(screen, 'blue', blue_fruit)
